...
- Provide a standalone public API (`FeedbackMail.openComposer(to:subject:body:)`) that lets the user send feedback directly to the developer, pre-addressed to a developer-configured support email with device/app version info pre-filled (`FeedbackMail.diagnosticsBody(extra:)`), or a minimal bundled in-SDK feedback form (`FeedbackFormView`) as a lighter-weight alternative. Implemented via a `mailto:` link rather than `MFMailComposeViewController`/`NSSharingService` — see MILESTONES.md M9 for why.
